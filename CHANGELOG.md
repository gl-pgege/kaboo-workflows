...
- the `obo` MCP auth strategy and `OBOTokenAuth` are gone.
`build_auth("obo", ...)` now raises ValueError, and `type: obo` fails config
validation.

### Feat

- **auth**: remove the obo strategy

## v0.19.0 (2026-08-26)

### Feat

- send an outbound token in several headers
...
- Langfuse eval push uses ``dataset.run_experiment`` so runs appear as v4 experiments
...
- register the ``run_succeeded`` eval scorer (class existed, ``build_scorer`` did not)

## v0.17.0 (2026-08-16)

### Feat

- first-class evaluation — OTel tracing, feedback trace ids, golden-dataset eval runner

## v0.16.0 (2026-08-12)

### Feat

- capture cache reads and dollar cost in AGENT_COMPLETE usage

## v0.15.3 (2026-08-11)

### Fix

- make the end-of-run activity flush deterministic

## v0.15.2 (2026-08-11)

### Fix

- flush queued activity before the terminal run event

## v0.15.1 (2026-08-11)

### Fix

- count the entry agent's token usage in the run rollup

## v0.15.0 (2026-08-11)

### Feat

- accumulate token usage into activity snapshots
- escape literal dollar-brace in config interpolation
- configurable timeout for streamable-http MCP transports
